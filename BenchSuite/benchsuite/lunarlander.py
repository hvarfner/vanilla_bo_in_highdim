...
class LunarLanderBenchmark(Benchmark):
    def __init__(
        ...
        super().__init__(
            12,
            torch.zeros(12, dtype=settings.DTYPE, device=settings.DEVICE),
            torch.ones(12, dtype=settings.DTYPE, device=settings.DEVICE) * 2
        )
    ...
    def demo_heuristic_lander(
        ...
        with self.lock:
            ...
            for step in range(max_steps):
                a = self.heuristic_Controller(s, w)
    ...
    def __call__(
        ...
        rewards = []
        for y in x:
            reward = self.demo_heuristic_lander(y)
